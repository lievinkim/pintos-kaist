 ...
 * 부팅 직후 최초 메모리
 * - 핀토스는 가상 메모리를 사용
 * - 모든 응용 프로그램은 가상메모리를 사용 시 4GB를 할당 (실제 메모리 X)
 * - 4GB 중 3GB은 사용자 프로그램이 사용하고 1GB은 커널(핀토스)이 사용
 * 
 * 1GB 커널공간 배치
 * - .bss : 초기화값이 저장되지 않은 전역변수들의 위치
 * - .data : 초기화된 전역변수들의 위치
 * - .rodata : 초기화된 전역변수 중 값이 바뀌지 않는 변수들의 위치(= 읽기전용)
 * - .text : main 함수 컴파일해서 나오는 값들
 * - 위 4개는 우리가 보고 있는 코드들이 위치해 있는 공간
 * 
 * LOADER_PhYS_BASE(값) = 값은 커널의 시작 지점을 의미
 * 
 */

/* 동적 메모리 할당 영역 초기화 */
/* 노트. 초기화 후 main에서 paging_init 함수를 통해 페이징 시스템을 초기화 */
